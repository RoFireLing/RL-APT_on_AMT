...
import labprograms.newStrategy.utl.RLAPT_DRT;
...
public class RLAPT_drt implements NewStrategy {
    ...
    public void testing(String objectName, int repeatTimes) {
        ...
        for (int i = 0; i < Constant.repeatNumber; i++) {
            System.out.println("RL-APT4" + objectName + "使用drt:" +
                    "执行第" + String.valueOf(i + 1) + "次测试：");

            //初始化DRT
            RLAPT_DRT drt = new RLAPT_DRT();

            drt.setParameters4DRT(objectName);

            //初始化测试剖面
            drt.initializeDRT(Constant.getPartitionNumber(objectName));
...
//                            String resultRelation = GetResultRelation4MOS.getResultRelation(sourceResult,followUpResult);
        ...
        //记录均值结果方便查看
        String txtLogName = "DRT4" + objectName + ".txt";
        ...
        txtLogName = "DRT4" + objectName + "_Contents.txt";
...
//
//        String excelLogName = "RLAPT_drt4" + objectName + ".xlsx";
