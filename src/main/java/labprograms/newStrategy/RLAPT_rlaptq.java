...
import labprograms.newStrategy.utl.RLAPT_Q;
...
public class RLAPT_rlaptq implements NewStrategy {
    ...
    public void testing(String objectName, int repeatTimes) {
        ...
        for (int i = 0; i < Constant.repeatNumber; i++) {
            System.out.println("RL-APT4" + objectName + "使用rlapt_q:" +
                    "执行第" + String.valueOf(i + 1) + "次测试：");

            //初始化RL APT
            RLAPT_Q rlapt = new RLAPT_Q();
...
//
//        String excelLogName = "RLAPT_rlaptq4" + objectName + ".xlsx";
